...
Наибольшим вызовом в проекте стало обеспечение высокой производительности с заданными метриками: **1000 запросов в секунду (RPS) при времени отклика (SLI) не более 50 миллисекунд**. К сожалению, провести полномасштабное нагрузочное тестирование на домашнем ноутбуке не смог так, как метрики сильно прыгали при нескольких запусках.
Так же учитывая использование транзакций в проекте, не успел завершить все запланированные оптимизации. В планах было внедрение кэширования с использованием Redis и асинхронная обработка операций – например, немедленный ответ пользователю с последующей асинхронной записью в базу данных. Хотя идей по улучшению производительности было много, к сожалению времени не хватило на всё.

Ещё в процессе разработки пришлось принимать решения по обработке различных краевых случаев, которые не были явно описаны в условии. Например:

- Обработка неразрешенных HTTP-методов для эндпоинтов
- Валидация транзакций с отрицательным количеством монет
...
Все эти сценарии реализованл, но не уверен, что все правильно.
